...
> **Where to vote** — live votes live at **`/vote`** (embedded FOR/AGAINST/ABSTAIN ballots, live tallies, countdown), and the same page's past-votes archive keeps every prior decision browsable — the Foundational Governance Election (full record at `/governance-vote`) and all finalized proposals, with outcomes, tallies, and quorum. Aggregated outcomes also live at `/results`. Admin safety valves (2026-09-23): a live vote can be **paused** (ballots refused while paused; the deadline is extended by the paused duration on resume so voters lose no time), **resumed**, or **stopped** early — a stopped vote closes immediately and is finalized by the normal outcome rules (quorum unmet → EXPIRED, majority → PASSED/FAILED). Every control is audit-logged.
