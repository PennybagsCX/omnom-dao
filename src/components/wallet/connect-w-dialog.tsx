...
export function ConnectWalletDialog({
  ...
  const handleClose = useCallback(
    (openState: boolean) => {
      if (!openState) {
        // If the user bails mid-flow on a "not in snapshot" or error, disconnect
        // the wallet so the header doesn't show a connected-but-unverified state.
        if (
          isConnected &&
          (phase === "not-in-snapshot" || phase === "error" || phase === "rejected")
        ) {
          disconnect();
        }
      }
...
function IdlePhase({ onDevConnect }: { onDevConnect: () => void }) {
  // Enhanced dev wallet detection: Check for window.enhancedMockWalletLoaded flag
  const isDev = process.env.NODE_ENV === "development" && (
    isDevMockWalletActive() || 
    (typeof window !== "undefined" && (window as unknown as { enhancedMockWalletLoaded?: boolean }).enhancedMockWalletLoaded)
  );
