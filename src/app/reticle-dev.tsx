  ...
  useEffect(() => {
    ...
    void import('@reticlehq/react').then(
      ({ reticle, install, registerCapabilities }) => {
        ...
        reticle.connect({
          projectId: 'omnom-dao-40f8d66c', ...(url ? { url } : {}),
          ...(token ? { token } : {}),
          ...(root ? { root } : {}),
        });
        ...
        registerCapabilities({
          testids: ['emoji-bar', 'stub-dialog', 'proposals-unlock-banner', 'proposals-locked-banner', 'save-draft-button', 'results-proposal-row', 'past-vote-fge', 'proposal-vote-ballot', 'proposal-vote-results', 'countdown-timer'],
