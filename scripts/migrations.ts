/**
 * OMNOM DAO — Migration statements (single source of truth).
 *
 * Pure data module: no side effects, safe to import from tests. The runnable
 * script lives in `scripts/migrate.ts`; integration tests apply these
 * statements to a real in-memory libsql client so route SQL is validated
 * against the actual schema (catches bugs like selecting nonexistent columns,
 * which the hand-mocked `db.execute` tests cannot see).
 */

export const MIGRATION_STATEMENTS: Array<{ sql: string }> = [
  // ── 1. Users ─────────────────────────────────────────────
  {
  ...
  //    (approve/reject/record-outcome/status overrides/deletes + live-vote
  //    pause/resume/stop), read via GET /api/v1/audit-log and written by
  //    src/lib/audit-log.ts. No FK on target_id — it may reference any
  //    entity kind (and PROPOSAL_DELETED entries outlive the deleted
  //    proposal).
  {
